      ...
      <div style="margin-top:8px;">
        ...
        ${payments.map(pay => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid #f0f0f0;font-size:0.85rem;">
            ...
            ${canEdit ? `<button class="btn red small-btn" onclick="deletePurchasePayment('${esc(p.id)}','${esc(pay.id)}')" style="padding:2px 7px;font-size:0.75rem;" title="Delete Payment" aria-label="Delete Payment">✕</button>` : ""}
          </div>`).join("")}
      </div>` : "";
    ...
    return `<div class="client-card">
      <div class="card-header-row">
        ...
        <div class="card-actions">
          ...
          ${canEdit ? `<button class="btn blue small-btn" onclick="openEditPurchaseModal('${esc(p.id)}')" title="Edit" aria-label="Edit">✏️</button>` : ""}
          ${canEdit ? `<button class="btn red small-btn" onclick="deletePurchase('${esc(p.id)}')" title="Delete" aria-label="Delete">🗑️</button>` : ""}
        </div>
      </div>
...
function _purchaseRowHTML() {
  ...
  return `<input class="pi-name" placeholder="Item name" style="flex:2;min-width:130px;margin:0;">
    ...
    <button class="btn red small-btn" onclick="removePurchaseRow(this)" style="margin:0;" title="Remove row" aria-label="Remove row">✕</button>`;
}
...
function _editPurchaseRowHTML(item = {}) {
  ...
  return `<div class="purchase-item-row">
    ...
    ${invBadge}
    <button class="btn red small-btn" onclick="removeEditPurchaseRow(this)" style="margin:0;" title="Remove row" aria-label="Remove row">✕</button>
  </div>`;
}
